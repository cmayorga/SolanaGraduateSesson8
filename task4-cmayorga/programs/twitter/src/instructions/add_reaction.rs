...
pub fn add_reaction(ctx: Context<AddReactionContext>, reaction: ReactionType) -> Result<()> {
    ...
    let new_reaction = &mut ctx.accounts.tweet_reaction;
    ...
    new_reaction.reaction_author = author.key();
    new_reaction.parent_tweet = tweet.key();
    new_reaction.bump = ctx.bumps.tweet_reaction;
